...
export function SnapshotsModal({ isOpen, onClose, currentTransactions, currentBudgets, onLoadSnapshot }: SnapshotsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md transition-all duration-500 animate-in fade-in" dir={dir}>
      ...
      >
        <div className="p-6 sm:p-8 border-b border-white/5 flex flex-row-reverse items-center justify-between relative">
           <button 
            onClick={onClose} 
            className="text-slate-400 hover:text-white transition-all bg-white/5 hover:bg-white/10 p-2 sm:p-2.5 rounded-xl border border-white/10"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="p-6 sm:p-8 overflow-y-auto flex-1 custom-scrollbar">
          <form onSubmit={handleSaveSnapshot} className="flex flex-col gap-4 mb-8 sm:mb-10 group">
            <div className="flex flex-col gap-3">
              <label htmlFor="snapshot-name" className="text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <input 
                  ...
                  className="flex-1 px-5 sm:px-6 py-3.5 sm:py-4 rounded-xl sm:rounded-2xl border border-white/10 bg-white/[0.02] text-white focus:ring-2 focus:ring-indigo-500/30 outline-none transition-all shadow-inner placeholder:text-slate-600 font-bold text-sm"
                />
                <button 
                  type="submit"
                  className="flex items-center justify-center gap-2 px-8 py-3.5 sm:py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-xl sm:rounded-2xl transition-all shadow-xl shadow-indigo-600/20 whitespace-nowrap uppercase tracking-widest text-[10px] sm:text-xs"
                >
          ...
          <div className="space-y-6">
            ...
            {snapshots.length === 0 ? (
              <div className="text-center py-12 sm:py-16 bg-white/[0.02] rounded-2xl sm:rounded-[2rem] border border-dashed border-white/5">
                <Clock className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4 text-slate-700 opacity-20" />
                <p className="text-slate-500 font-bold uppercase tracking-widest text-[10px] sm:text-xs">{language === 'ar' ? 'لا توجد نسخ محفوظة حالياً.' : 'No saved backups yet.'}</p>
              </div>
            ) : (
              snapshots.map(snapshot => (
                <div key={snapshot.id} className="group flex flex-col sm:flex-row sm:items-center justify-between p-5 sm:p-6 rounded-2xl sm:rounded-[2rem] border border-white/5 bg-white/[0.01] hover:bg-white/[0.03] hover:border-indigo-500/30 transition-all duration-300 relative overflow-hidden gap-4">
                  ...
                  <div className="relative z-10">
                    <h4 className="font-black text-base sm:text-lg text-white mb-1.5 sm:mb-2 group-hover:text-indigo-300 transition-colors">{snapshot.name}</h4>
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3">
                       <p className="text-[9px] sm:text-[10px] font-black bg-white/5 px-2.5 sm:px-3 py-1 rounded-full text-slate-500 uppercase tracking-tighter" dir="ltr">
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 relative z-10 w-full sm:w-auto">
                    <button
                      ...
                      className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 sm:px-5 py-2.5 bg-emerald-500/10 hover:bg-emerald-500 text-emerald-400 hover:text-white font-black rounded-xl border border-emerald-500/20 transition-all text-[10px] sm:text-[11px] uppercase tracking-widest shadow-lg shadow-emerald-500/5"
                    >
                    ...
                    <button
                      ...
                      className="p-2.5 sm:p-3 text-slate-500 hover:text-rose-400 hover:bg-rose-500/10 rounded-xl transition-all border border-transparent hover:border-rose-500/20"
                    >
